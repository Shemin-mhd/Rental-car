...
export default function BookingStatusPage() {
    ...
    useEffect(() => {
        ...
        newSocket.on("bookingStatusUpdate", (data: any) => {
            setPendingBookings(prev => prev.map(bk =>
                bk._id === data.bookingId
                    ? { ...bk, documentStatus: data.documentStatus, status: data.status }
                    : bk
            ));
        });
    ...
    // 🛰️ Join rooms for active bookings
    useEffect(() => {
        if (socket && pendingBookings.length > 0) {
            pendingBookings.forEach(bk => {
                socket.emit("join", bk._id);
            });
        }
    }, [socket, pendingBookings]);
